...
import { V3_LISTING_CATEGORY_SLUGS } from "@shared/v3-listing";
...
export const appRouter = router({
  ...
  marketplace: router({
    ...
    updateV3OwnerJumiaOrder: publicProcedure.input(z.object({ orderId: z.string().uuid(), status: z.enum(["placed", "confirming", "accepted", "sourcing", "ready", "out_for_delivery", "completed", "cancelled"]), quotedAmount: z.number().nonnegative().max(10_000_000).optional(), paymentStatus: z.enum(["not_due", "paid", "refund_due", "refunded"]).optional(), fulfilmentMethod: z.enum(["siaya_pickup", "home_delivery", "collection_point"]).optional(), preferredLocation: z.string().trim().max(180).optional(), deliverySchedule: z.string().trim().max(120).optional(), ownerNotes: z.string().trim().max(3_000).optional(), cancellationReason: z.string().trim().max(600).optional() })).mutation(({ ctx, input }) => updateV3OwnerJumiaOrder(ctx.supabaseIdentity, input)),
