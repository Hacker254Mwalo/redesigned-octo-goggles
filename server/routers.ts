...
export const appRouter = router({
  ...
  marketplace: router({
    ...
    submitV3VendorProduct: publicProcedure.input(z.object({ title: z.string().trim().min(3).max(180), categorySlug: z.enum(V3_LISTING_CATEGORY_SLUGS), price: z.number().positive().max(10_000_000), stockQuantity: z.number().int().min(1).max(100_000), imageData: z.string().max(7_000_000), imageType: z.enum(["image/jpeg", "image/png", "image/webp"]) })).mutation(({ ctx, input }) => submitV3VendorProduct(ctx.supabaseIdentity, input)),
