...
export async function listV3ModerationProducts(identity: SupabaseIdentity | null) {
  ...
  const { data, error } = await getSupabaseServiceClient().from("products").select("id,title,description,category_slug,stock_quantity,image_url,vendor_id,final_price,allow_pay_on_pickup,livestock_type,livestock_details,livestock_welfare_attested,livestock_movement_acknowledged,status,created_at").in("status", ["PENDING", "ACTIVE", "FLAGGED"]).order("created_at", { ascending: true });
