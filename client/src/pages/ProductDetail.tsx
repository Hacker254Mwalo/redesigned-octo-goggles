...
export default function ProductDetail() {
  ...
  const saveBuyerPhone = trpc.marketplace.saveV3BuyerPhone.useMutation({ onSuccess: () => { setBuyerPhone(""); buyerOrderAccess.refetch(); toast.success("Your hub-pickup contact is saved."); }, onError: error => toast.error(error.message) });
  ...
  const confirmHubOrder = () => {
    ...
    if (!buyerOrderAccess.data?.hasPhone) return toast.error("Save your Kenyan order contact number before confirming hub pickup.");
    confirmV3Order.mutate({ productId: product.id });
  };
  const saveHubPhone = () => {
    if (!session) return toast.error("Sign in with your verified email session before saving an order contact number.");
    saveBuyerPhone.mutate({ phone: buyerPhone });
  };
