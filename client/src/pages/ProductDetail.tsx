...
export default function ProductDetail() {
  ...
  const confirmHubOrder = () => {
    ...
    if (!buyerOrderAccess.data?.hasName || !buyerOrderAccess.data?.hasPhone) return toast.error("Save your name and Kenyan order contact before confirming hub pickup.");
    confirmV3Order.mutate({ productId: product.id });
  };
  const saveBuyerDetails = () => {
    if (!session) return toast.error("Sign in with your verified email session before saving protected pickup details.");
