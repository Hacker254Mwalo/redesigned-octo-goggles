...
export default function JumiaStorePage() {
  ...
  function selectResult(result: { title: string; url: string; snippet: string; imageUrl: string | null; price: number | null; currency: string | null }) {
    ...
    <div className="mx-auto max-w-6xl px-5 py-12 sm:py-16">
      <section className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
        <div>
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground">Explore products from Jumia Kenya, add your choices to one MtaaMarket order, and select collection or home delivery at checkout. Pay when you receive it.</p>
          <div className="mt-6 flex flex-wrap gap-3 text-sm text-[#245441]"><span className="inline-flex items-center gap-2 rounded-full bg-[#e8f4ec] px-3 py-2"><ShieldCheck size={16} /> Clear pricing</span><span className="inline-flex items-center gap-2 rounded-full bg-[#e8f4ec] px-3 py-2"><Package size={16} /> Pay on receipt</span></div>
        </div>
        <aside className="rounded-3xl border border-[#cfe3d7] bg-[#f4faf5] p-6"><div className="flex items-center gap-3 text-[#0e6c53]"><Truck size={24} /><p className="eyebrow !text-[#0e6c53]">Siaya delivery</p></div><p className="mt-4 text-lg font-semibold text-[#0e2f27]">Everything you need, from search to delivery.</p><p className="mt-3 text-sm leading-6 text-[#35584a]">Choose the product, quantity, and hand-off option that works for you. Your order details stay together from checkout to delivery.</p></aside>
      </section>

      {submittedOrder ? <section className="mt-10 rounded-3xl border border-[#b9dcc5] bg-white p-6 shadow-sm sm:p-8" aria-live="polite"><div className="flex items-start gap-4"><div className="rounded-full bg-[#e8f4ec] p-3 text-[#0e7c5a]"><CheckCircle2 size={25} /></div><div><p className="eyebrow">Order received</p><h2 className="mt-1 text-2xl font-semibold">{submittedOrder.order_number} is being prepared.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Your order has been received. Payment is due when you collect it or when it arrives. You can follow its progress from your account.</p></div></div><div className="mt-6 flex flex-wrap gap-3"><button className="primary-cta" type="button" onClick={() => setSubmittedOrder(null)}>Place another order <ArrowRight size={17} /></button><Link className="secondary-cta" href="/dashboard">View my orders</Link></div></section> : <form className="mt-10 grid gap-8 lg:grid-cols-[1fr_0.78fr]" onSubmit={submit}>
        <section className="rounded-3xl border bg-white p-6 shadow-sm sm:p-8">
          <div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Step 1</p><h2 className="mt-1 text-2xl font-semibold">Find what you want</h2></div><Search className="text-[#1b6a55]" /></div>
          <p className="mt-3 text-sm leading-6 text-muted-foreground">Search Jumia products, compare options, and add your choice to the basket.</p>
