...
const statusCopy: Record<string, string> = {
  placed: "Order placed",
  confirming: "Founder confirming item",
  accepted: "Accepted for fulfilment",
  sourcing: "Being sourced through JForce",
    ...
    <div className="mx-auto max-w-6xl px-5 py-12 sm:py-16">
      <section className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
        <div>
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground">Order the Jumia items you want through MtaaMarket. Add your item details, schedule collection or home delivery, and submit the order without paying first. The founder fulfils the order through JForce and updates you here.</p>
        ...
        <aside className="rounded-3xl border border-[#cfe3d7] bg-[#f4faf5] p-6"><div className="flex items-center gap-3 text-[#0e6c53]"><Truck size={24} /><p className="eyebrow !text-[#0e6c53]">Founder-operated fulfilment</p></div><p className="mt-4 text-lg font-semibold text-[#0e2f27]">You place the customer order here. The founder handles the JForce step behind the scenes.</p><p className="mt-3 text-sm leading-6 text-[#35584a]">The final item, amount, availability, and hand-off route are confirmed before purchase. If an item is unavailable, the founder can cancel the order.</p></aside>
      </section>

      {submittedOrder ? <section className="mt-10 rounded-3xl border border-[#b9dcc5] bg-white p-6 shadow-sm sm:p-8" aria-live="polite"><div className="flex items-start gap-4"><div className="rounded-full bg-[#e8f4ec] p-3 text-[#0e7c5a]"><CheckCircle2 size={25} /></div><div><p className="eyebrow">Order received</p><h2 className="mt-1 text-2xl font-semibold">{submittedOrder.order_number} is in the fulfilment queue.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">The founder will confirm the Jumia item and proceed through JForce. No payment was collected. You will pay at collection or delivery after the parcel and hand-off are confirmed.</p></div></div><div className="mt-6 flex flex-wrap gap-3"><button className="primary-cta" type="button" onClick={() => setSubmittedOrder(null)}>Place another order <ArrowRight size={17} /></button><Link className="secondary-cta" href="/dashboard">View my orders</Link></div></section> : <form className="mt-10 grid gap-8 lg:grid-cols-[1fr_0.78fr]" onSubmit={submit}>
        <section className="rounded-3xl border bg-white p-6 shadow-sm sm:p-8">
          ...
          <p className="mt-3 text-sm leading-6 text-muted-foreground">Search by item name and describe the exact option you want. This creates a normal order request without requiring a product link or a separate vendor account.</p>
