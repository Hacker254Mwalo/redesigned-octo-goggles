...
const modeCopy: Record<AccountMode, { title: string; description: string }> = {
  ...
  signup: { title: "Create your account", description: "Create one account with your email and a secure password. We will send a verification code only for this new account." },
  recovery: { title: "Reset your password", description: "We will send a verification code to confirm it is you before you choose a new password." },
};
...
export function MtaaAccountDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const verifyCode = async () => {
    if (!challenge || !isMtaaMarketAccountCode(emailCode)) {
      setNotice("Enter the six- or eight-digit code from the genuine MtaaMarket email.");
      return;
    }
    setSubmitting(true);
    setNotice("");
    try {
      ...
      setNotice("That code could not be verified. Check that it is the newest code, then try again or request another code.");
    } finally {
      setSubmitting(false);
    }
  };
