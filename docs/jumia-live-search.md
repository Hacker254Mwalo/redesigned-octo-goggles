...
## Final catalogue presentation notes — 3852e67

The catalog-first storefront now opens with the real `Android smartphone` query rather than a broad or year-specific placeholder. The live adapter deduplicates identical normalized displayed titles after quality scoring, retaining the best-ranked result while keeping distinct model and variant titles. The final production browser check returned eight concrete phone choices for the default query, with honest missing-price and missing-photo states where public indexed metadata was unavailable. These changes improve relevance and reduce repeated cards without inventing catalogue facts or claiming an official full-catalogue feed.
