...
## Commerce-first storefront releases — b06af4c and 8cbdc9b

Commit `b06af4c` replaced the landing-led catalogue hierarchy with a compact commerce header, a four-column live-result grid, a result count toolbar, verified-data filters, eight category shortcuts, and a compact basket utility. Commit `8cbdc9b` added a homepage live-selection shelf backed by the existing server-authorized public discovery procedure, exposing four real product cards above the honest local-listings empty state. Both releases passed the local TypeScript, Vitest, Vercel-build, production-build, and whitespace gates before deployment.

The immutable `8cbdc9b` deployment `https://siayaonlinemarket-39ytpgb0i-hacker254mwalos-projects.vercel.app` reached Vercel Production success. Browser verification showed nine live choices in the catalogue, `All choices`, `With photos`, and `Price shown` filters, eight category shortcuts, and a selection-to-basket-to-checkout flow. The `With photos` filter honestly returned zero choices when the live response had no trusted images. Adding a selected real result showed `Basket 1`, fulfilment controls, and a sign-in-gated order action; no order was submitted. The canonical alias was then checked and served the same four-card homepage shelf, nine-choice catalogue, new controls, and existing Owner session.
